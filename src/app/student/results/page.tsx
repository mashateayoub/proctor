    ...
    <div className="w-full">
        <div className="max-w-[1024px] mx-auto">
          ...
          <motion.div {...fadeUp} className="mb-12">
             <h1 className="text-section-heading text-ink mb-2 tracking-tight">Your Transcripts.</h1>
          ...
          {loading ? (
             <motion.p {...fadeIn} className="text-center text-caption text-ash py-12">Fetching secure records...</motion.p>
          ) : results.length === 0 ? (
             <motion.div {...fadeIn} className="text-center py-24 bg-white rounded-[12px]">
                <p className="text-body-standard text-ash">No published results exist on your transcript.</p>
             </motion.div>
          ) : (
            <motion.div
               ...
               {results.map((r) => (
                 <motion.div key={r.id} variants={staggerItem}>
                   <Card elevated className="p-8 bg-white" delay={0}>
                      ...
                      <h3 className="text-card-title text-ink mb-2 leading-tight">
                         {r.exams?.exam_name}
                      </h3>
                      <p className="text-body-standard text-ash mb-6">
                         Duration: {r.exams?.duration_minutes}m
                      </p>

                      {r.coding_submissions && r.coding_submissions.length > 0 && (
                        <div className="border-t border-hairline pt-6">
                           <h4 className="text-[14px] font-semibold text-ink mb-4">Coding Component</h4>
                           {/* Teacher-controlled grade */}
                           <div className="flex items-center justify-between bg-soft-cloud p-4 rounded-[8px] mb-3">
                              <span className="text-[13px] font-medium text-ash">Instructor Grade</span>
                           ...
                           {r.coding_submissions.map((sub: any, i: number) => (
                              <div key={i}>
                                {sub.testResults && sub.testResults.length > 0 ? (
                                  <div className="bg-soft-cloud rounded-[8px] p-4">
                                    <div className="flex justify-between items-center mb-3">
                                      <span className="text-[12px] font-semibold text-ash">Test Suite</span>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                      {sub.testResults.map((tr: any, j: number) => (
                                        ...
                                        >
                                          <span className={tr.passed ? 'text-green-600' : 'text-red-500'}>
                                            {tr.passed ? '✓' : '✗'}
                                          </span>
                                          <span className="text-ash">{tr.label}</span>
                                        </motion.div>
                                      ))}
                                    </div>
                                  </div>
                                ) : (
                                  <div className="flex justify-between items-center p-3 rounded-[8px] text-[11px] text-mute">
